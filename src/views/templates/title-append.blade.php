@if (isset($filterData) && $filterData)
    <button type="button" class="with-tooltip material-icons title-btn {{ $filterData['hasFilters'] ? 'active' : '' }}" id="title-btn-filter" title="{{ trans('clumsy::titles.filters') }}" data-toggle="collapse" data-target="#filter-collapse">&#xE152;</button>
@endif
